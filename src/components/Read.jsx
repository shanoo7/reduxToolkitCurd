            ...
            .map((ele, index) => (
              <div key={ele?.id || index} className="bg-white p-6 rounded-xl shadow-lg flex flex-col items-center text-center">
                ...
                <div className="flex gap-2">
                  <Link to={`/update/${ele?.id}`}><button className="bg-blue-700 font-bold text-white px-4 py-2 rounded-lg hover:bg-blue-700">Edit</button></Link>
                  <button onClick={() => dispatch(deleteUser(ele?.id))} className="bg-red-700 font-bold text-white px-4 py-2 rounded-lg hover:bg-red-700">Delete</button>
                  <button onClick={() => [setShowModal(true), setId(ele?.id)]} className="bg-green-700 font-bold text-white px-4 py-2 rounded-lg hover:bg-green-700">View</button>
                </div>
              </div>
            ))}
        </div>
